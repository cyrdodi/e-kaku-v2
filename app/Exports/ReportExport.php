...
class ReportExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithCustomStartCell, WithStyles
{
  ...
  public function map($cetakTrans): array
  {
    return [
      ...
      strtoupper($cetakTrans->biodata->alamat) . ', ' . strtoupper($cetakTrans->biodata->kelurahan) . ', ' . strtoupper($cetakTrans->biodata->kecamatanName->name) . ', ' . strtoupper($cetakTrans->biodata->kabupaten) . ' - ' . strtoupper($cetakTrans->biodata->provinsi) . ' ' . $cetakTrans->biodata->kode_post,
      strtoupper($cetakTrans->biodata->kecamatanName->name),
